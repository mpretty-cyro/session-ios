...
for i in "${!TARGET_ARCHS[@]}"; do
    ...
    if [ $? -ne 0 ]; then
      ALL_ERROR_LINES=($(grep -n "error:" "${TARGET_BUILD_DIR}/libLokinet/liblokinet_output.log" | cut -d ":" -f 1))

      for e in "${!ALL_ERROR_LINES[@]}"; do
        error_line="${ALL_ERROR_LINES[$e]}"
        error=$(sed "${error_line}q;d" "${TARGET_BUILD_DIR}/libLokinet/liblokinet_output.log")
        ...
        if [[ $error == *'CMake Error'* ]]; then
            ...
            error="${error}$(sed "${actual_error_line}q;d" "${TARGET_BUILD_DIR}/libLokinet/liblokinet_output.log")"
        fi

        # Exclude the 'ALL_ERROR_LINES' line and the 'grep' line
        if [[ ! $error == *'grep -n -i "error"'* ]] && [[ ! $error == *'grep -n -i error'* ]]; then
