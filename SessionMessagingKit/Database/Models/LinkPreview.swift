...
public extension LinkPreview {
    ...
    private static var previewUrlCache: LRUCache<String, String> = LRUCache()
    ...
    static func previewUrl(
        ...
        if let cachedUrl: String = sync({ await previewUrlCache.get(key: body) }) {
        ...
        guard let urlMatch: URLMatchResult = previewUrlMatches.first else {
            ...
            Task { await previewUrlCache.set(key: body, value: "") }
            return nil
        }
        ...
        Task { await previewUrlCache.set(key: body, value: urlMatch.urlString) }
